...
Acronym: **M**ixing (**H**) Enthalpy

This study computes the enthalpy of mixing between unary reference states and a multicomponent mixture as a function of temperature. For binary alloys, it is defined as $$\Delta H_\text{mix} = H_{AB} - (x_A H_A + x_B H_B)$$ and represents the amount of heat released ($\Delta H_\text{mix}<0$) or required $(\Delta H_\text{mix}>0)$ when forming a mixture from pure sources. 

When combined with the entropy of mixing $\Delta S_\text{mix}$, representing the number of microstates gained by mixing, the free energy is obtained which determines whether mixing is thermodynamically favorable. Mixing increases the entropy substantially because chemical disorder can now occur, affecting the configurational entropy, and the difference in mass, volume, and interatomic forces between alloy species changes the mode, multiplicity, and frequency of phonons, affecting the vibrational entropy. Reliably estimating the free energy is often a difficult task requiring multiple computational and experimental techniques.

Regardless, the enthalpy of mixing describes how changing the types of bonds relates to the energy and strain of the crystal. If $\Delta H_\text{mix}<0$, the system wants to increase the number of unlike bonds and is indicative of ordering. If instead $\Delta H_\text{mix}>0$, the system wants to remain separated as unlike bonds increase enthalpy. Finally, if $\Delta H_\text{mix}\approx 0$, the system has no bonding preference and entropy drives the system towards an ideal solution.

When running this study, at least one temperature is required. The use of a dataset is optional and other types of mixtures can be generated by specifying the `order` and `composition` parameters. If a dataset is used and multiple temperatures are specified, subdirectories must exist containing configurations for each temperature and the subdirectory names must match the specified temperature values (e.g., `dataset/800/` for 800K). Unlike other studies, since the pure reference states must also be simulated, lattice parameters `lattice`, `lattice_const`, `size` are always required and must be provided as lists where the first items correspond to each pure state in the order they appear in `composition` or the composition string in the comment lines of the dataset configurations. The last item should be the value for the mixture only when it is generated from scratch, as they are already specified in the comment lines of existing dataset configurations.

One final note: because pure and mixture states are allowed differ in size, the intensive form of mixing enthalpy is computed. 

### Workflow

1. Prepare pure and mixed configurations
2. Equilibrate to $T_i$ in the NPT ensemble
3. Repeat 1-2 for many configurations and compute $\langle H \rangle$ for each system
4. Repeat 1-3 for multiple temperatures
5. Compute $\Delta h_\text{mix}(T) = h_\text{mix} - \sum_i x_i h_i$ 
...
lattice: <list of lattice types [element 1, element 2, ..., mixture]>
lattice_const: <list of initial unit cell lengths [element 1, element 2, ..., mixture]>
size: <list of box lengths [element 1, element 2, ..., mixture]>
composition:
    <element 1 in potential file>: <atomic percentage as a whole number>
    <element 2 in potential file>: <atomic percentage as a whole number>
    ...
...
lattice: <list of lattice types [element 1, element 2, ...]>
lattice_const: <list of initial unit cell lengths [element 1, element 2, ...]>
size: <list of box lengths [element 1, element 2, ...]>
...
timestep: <MD timestep in ps>
temperature: <equilibration temperature as a single value or a list>
